...
import { init, refresh, getUser, source, REMOTE } from './state.js'
...
try {
  await init()
  const u = getUser()
  console.error(`[opengym-mcp] serving profile ${u.name} (${u.id}) from ${REMOTE ? 'remote ' : ''}${source()}`)
} catch (e) {
...
for (const t of TOOLS) {
  server.tool(
    ...
    async (params) => {
      try {
        // Remote mode refreshes here rather than inside the handlers, which keeps all eight of
        // them synchronous. Local mode is a no-op: getState() re-reads on mtime change.
        await refresh()
