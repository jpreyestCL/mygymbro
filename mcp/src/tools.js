...
import {
  setLabelIn, exLine, muscleName, policyName, friendlyDuration, ratio, muscleOrder
} from './labels.js'
import {
...
// A set in this fork carries the unit it was logged in (`u`), absent meaning the profile's.
// Every number that leaves this server is therefore normalised to the profile's unit first,
// and each payload names that unit. An LLM cannot see a missing `u` and will happily average
// 100 kg with 225 lb into "162.5" — so the conversion has to happen here, not in the prompt.
import { wBase, baseUnit } from '../../frontend/src/lib/units.js'
...
function entryView(e, S) {
  ...
  return {
    ...
    sets: (e.sets || []).map(s => ({
      ...
      warmup: isWarmupRow(s),
      // Both the label and the raw number speak the profile's unit, whatever the set was
      // logged in — see the units note at the top of this file.
      label: setLabelIn(S, e.id, { ...s, done: undefined }, cfg, baseUnit(S)),
      w: wBase(S, s),
      r: Number(s.r) || 0,
      sec: Number(s.sec) || 0,
      min: Number(s.min) || 0,
  ...
  for (const w of (S.workouts || [])) {
    for (const e of (w.entries || [])) {
      for (const s of (e.sets || [])) {
        // Warm-up rows are excluded for the same reason bestSetOf excludes them: a light row
        // at high reps can out-estimate the working set it was warming up for. Skipping them
        // here too keeps this table and the estimate_1rm tool from disagreeing.
        if (!s.done || isWarmupRow(s)) continue
        const load = wBase(S, s)
        const est = estimate1RM(load, s.r, formula)
        ...
        if (!prev || est > prev.est) {
          ...
          byId.set(e.id, { exId: e.id, exName: ex.n, bp: ex.bp || null, est, w: load, r: Math.round(Number(s.r)), date: w.d })
        }
      }
    }
  }
