...
> **This fork.** Sessions are Better Auth (Postgres), not cookie-HMAC / `db.json`. Coach
> routes `await` `readSession` / `requireAdmin`. The Android/iOS apps talk to the same API,
> so Coach is available on the phone when the user is signed into the instance — guests
> still do not see it. `node server.js` starts without Coach credentials; `/api/config`
> omits `coach` until an admin enables a provider (`publicConfig()` returns null).
