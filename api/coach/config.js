...
function key() {
  ...
  // Coach credentials are encrypted with a key derived from $DATA_DIR/secret. Sessions in this
  // fork no longer use that file (Better Auth has BETTER_AUTH_SECRET); we still mint one here
  // so an instance that never had the old cookie secret can store a provider token.
  ...
  try { secret = fs.readFileSync(secretFile, 'utf8').trim(); }
    ...
    try { fs.writeFileSync(secretFile, secret, { mode: 0o600 }); } catch { /* host may refuse */ }
  }
