 ...
 * are closures over push state and (in this fork) Better Auth, and passing them in keeps this
 * module free of a cycle (and trivially testable against fakes).
 *
 * Sessions are async here: readSession/requireAdmin await Better Auth, so every call is awaited.
 */
...
export function coachRoutes({ json, readBody, readSession, requireAdmin, audit }) {
  ...
  return {
    ...
    'POST /api/admin/coach/config': async (req, res) => {
      ...
      cfgStore.save(patch);
      audit?.(req, 'admin.coach.config', { user: admin });
      json(res, 200, { ok: true });
    },
    ...
    'POST /api/admin/coach/connect': async (req, res) => {
      ...
      audit?.(req, 'admin.coach.connect', { user: admin, msg: cfg.provider });
      json(res, 200, { ok: true });
    },
    ...
    'POST /api/admin/coach/disconnect': async (req, res) => {
      ...
      audit?.(req, 'admin.coach.disconnect', { user: admin });
      json(res, 200, { ok: true });
    },
