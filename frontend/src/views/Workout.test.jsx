...
// NOTE (this fork): startRest also carries labels for the iOS Lock Screen Live Activity —
// startRest(seconds, { workout, exercise, setsDone, setsTotal }). Upstream has no Live Activity
// and calls startRest(seconds) alone, so these assertions check the duration and the label shape
// rather than an exact one-argument call. The labels are worth pinning: they are what the
// countdown on a locked phone reads, and nothing else in the suite looks at them.
const expectRest = (seconds = 90) => {
  expect(mocks.startRest).toHaveBeenCalledWith(seconds, expect.objectContaining({
    exercise: expect.any(String),
    setsDone: expect.any(Number),
    setsTotal: expect.any(Number),
  }))
}
