...
import { effectiveRoutine, lastEntryFor, bestWeightFor, buildSets, freestyleConfig, defaultConfig, setsDoneActive, supersetUnits, unitOf, setLabel, setLabelIn, modeOf, isBw, isPerSide, sideReps, repStep, EFFORT, effortOf, stepEffort, capEffort, cascadeWeight, insertWarmupRow, removeRowAt, pairAdjacent, unpairSuperset, cleanupSg } from '../lib/history.js'
...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onAddWarmup, onRemoveSetAt, onStartTimed, onUnit, onPairPrev, onPairNext }) {
  const S = useStore(s => s.S)
...
function ActiveWorkout() {
  ...
  const setField = (idx, i, field, v) => mutEntry(idx, e => {
    ...
    // Changing a weight cascades to the following sets of the same phase, so a heavier bar
    // carries through the set instead of retyping every row. Safe across units: every row of
    // one exercise is displayed in that exercise's unit, so the copied number is exact.
  ...
  const confirmRemoveExercise = idx => {
    ...
    confirmSheet({
      ...
      confirmText: t('Remove'), danger: true, onConfirm: () => removeActiveExercise(idx)
    })
  }
  ...
  const toggle = (idx, i) => {
    ...
    // Rest and navigation are decided after the write, against fresh state, because pairing an
    // exercise mid-session can change which unit `idx` belongs to. Only progress beyond this
    // exercise's high-water mark counts: unchecking and re-checking finished work must not
    // replay the rest timer or jump the screen somewhere else.
    ...
    // Labels for the iOS Lock Screen countdown; ignored everywhere the activity is not shown.
    const restLabels = () => ({
      workout: fresh.name, exercise: exOr(fresh.entries[idx].id).n,
      setsDone: setsDoneActive(fresh), setsTotal: fresh.entries.reduce((n, e) => n + e.sets.length, 0),
    })
    ...
    if (!freshUnit || freshUnit.length <= 1) {
      if (!freshUnitDone) startRest(S.restSec, restLabels())
      return
    }
    ...
    if (step.unitDone) {
      if (!freshLastUnit) {
        ...
        startRest(S.restSec, restLabels())
      }
    } else {
      if (step.nextIdx != null) update(s => { if (s.active) s.active.cur = step.nextIdx })
      // A superset rests once the round is through both exercises, not between them.
      if (step.roundDone) startRest(S.restSec, restLabels())
    }
  }
        ...
        <div className="ss-card">
          ...
          {unit.map((idx, k) => <div key={idx} ref={el => { exRefs.current[idx] = el }} className="ss-ex" data-exidx={idx}>
            ...
            <ExerciseBlock entryIdx={idx} compact
              onToggle={i => toggle(idx, i)} onField={(i, f, v) => setField(idx, i, f, v)} onAddSet={() => addSet(idx)} onRemoveSet={() => removeSet(idx)} onAddWarmup={() => addWarmup(idx)} onRemoveSetAt={i => removeSetAt(idx, i)} onStartTimed={i => startTimed(idx, i)} onUnit={u => setExUnit(idx, u)} />
          </div>)}
        </div>
      ) : (
        <ExerciseBlock entryIdx={cur} onToggle={i => toggle(cur, i)} onField={(i, f, v) => setField(cur, i, f, v)} onAddSet={() => addSet(cur)} onRemoveSet={() => removeSet(cur)} onAddWarmup={() => addWarmup(cur)} onRemoveSetAt={i => removeSetAt(cur, i)} onStartTimed={i => startTimed(cur, i)} onUnit={u => setExUnit(cur, u)} onPairPrev={onPairPrev} onPairNext={onPairNext} />
      )}
