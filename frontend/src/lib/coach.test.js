...
describe('gating', () => {
  ...
  it('stays available on this fork’s mobile apps when the user is signed in', () => {
    expect(coachAvailable({ coach: { enabled: true } }, { id: 'u' }, { mobile: true })).toBe(true)
    expect(coachAvailable({ coach: { enabled: true } }, null, { mobile: true })).toBe(false)
    expect(coachAvailable(null, { id: 'u' }, { mobile: true })).toBe(false)
  })
