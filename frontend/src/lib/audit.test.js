...
// Every event name the server can emit, read out of api/server.js rather than restated here.
// A hardcoded list is the thing this test exists to prevent: it drifts silently, and then the
// dashboard prints a dotted identifier at a person. Two shapes to collect:
//   - literal calls, `audit(req, 'admin.user.disable', …)`
//   - the AUTH_EVENTS table, whose base names the server suffixes with .ok / .fail
...
const EVENTS = [...new Set([
  ...literalEvents,
  ...authBases.flatMap(base => [base + '.ok', base + '.fail']),
])]

// Reason codes: the fixed vocabulary the server stores in `msg` on a failure.
const REASONS = [
  ...
  ...[...SERVER.matchAll(/'http-' \+ res\.statusCode/g)].length
    ? ['http-400', 'http-401', 'http-403', 'http-404', 'http-409', 'http-422', 'http-429', 'http-500']
    : [],
]

describe('auditLabel', () => {
  it('actually found the server\u2019s events (guards the scrape itself)', () => {
    // A regex that silently matches nothing would make every assertion below vacuous.
    expect(EVENTS.length).toBeGreaterThan(10)
    expect(EVENTS).toContain('admin.denied')
    expect(EVENTS).toContain('auth.login.ok')
