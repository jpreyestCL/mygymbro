...
// i18n-core rather than i18n: this module is imported from plain Node by the MCP server,
// where `import.meta.glob` does not exist. The core half carries `t` without the Vite-only
// lazy loading.
...
// Node; the guard keeps this module loadable without Vite (the MCP server imports it).
// The defaults are absolute on purpose (upstream #79). A bare 'img/' resolves against the
...
// and nothing in the console. The mobile build overrides both with a CDN, so the leading
// slash never reaches capacitor://localhost.
