...
//
// This fork's Android/iOS apps talk to the Better Auth API, so a signed-in server user on
// the phone must see the Coach. `mobile` is accepted so existing call sites keep compiling,
// and ignored for gating. Guests without `user` still don't see it.
export const coachAvailable = (config, user, { demo, mobile: _mobile } = {}) =>
